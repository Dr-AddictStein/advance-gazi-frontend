...
import confrimcon from "../assets/confrim.png";
...
const CustomCheckbox = ({ checked, onChange, label, subtext }) => (
  <div className="flex pt-4 gap-3">
    <div
      onClick={onChange}
      className={`w-6 h-6 rounded-md border-2 ${
        checked ? "bg-[#F3F5F6] border-[#4086C4]" : "border-gray-400"
      } flex items-center justify-center cursor-pointer`}
    >
      {checked && <Check size={16} className="text-[#1D4781]" />}
    </div>
...
export default function Signup() {
  ...
  return (
    <div className="flex flex-col justify-between items-center bg-[#F3F5F6] w-[420px] h-auto rounded-[40px] mx-auto my-5 shadow-lg">
      ...
      {/* Heading */}
      <div className="flex items-center py-8 px-1 gap-2">

   <Link href={'/signup'}><Image src={icon} alt="backbutton" className="w-[40px] h-[40px]"/></Link>
<h1 className="font-extrabold text-[18px] text-center text-[#120405]">Please Select Wrong User Detail</h1>
</div>
